...
- **ドメイン知識のSubagent化（スキルの深化）**: リサーチにより得られた知見をスキルとして動的に抽出し、以後のリサーチで当該ドメインの質問が来た際にWeb検索をスキップして直接Subagentが回答、あるいは既存スキルの洗練・蓄積を行うトピックベースの安定IDアルゴリズムを採用。
...
- **インテリジェント・キャッシュ**: LLMの応答を永続化しつつ、12時間のTTL（有効期限）を設けることで、同一セッション内での効率化と、日を跨いだ際の情報鮮度の維持を両立。
