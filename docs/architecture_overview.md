...
### 安定性とセキュリティ
1. **スレッドセーフなファイル提供**: 最終レポートやネットワーク図は、各セッションの `thread_id` に紐づくユニークな一時ディレクトリ（`/tmp/g_seek/{thread_id}/`）に保存される。これにより、複数のユーザーが同時にリサーチを行った場合でも、ファイルの混同や上書きを防ぎ、セッション間の分離が保証される。
2. **フロントエンドエラーの回避**: Chainlit フロントエンドにおける `startsWith` エラーを回避するため、`cl.File` には必ず `os.path.abspath` で解決された絶対パスが渡されるよう設計されている。
3. **進捗管理**: `UIProgressManager` が各セクションの進捗を個別の `cl.Step` として管理し、並列処理の状況を整理して表示する。
